...
m.redirect = luci.dispatcher.build_url("admin", "wizard", "network")
m:chain("wireless")
...
if not net then
	luci.http.redirect(luci.dispatcher.build_url("admin/wizard/network"))
	return
end
...
if m:formvalue("cbid.network.%s._switch" % net:name()) then
	...
	if proto then
		...
		-- reload page
		luci.http.redirect(luci.dispatcher.build_url("admin/wizard/network", arg[1]))
		return
	end
end
