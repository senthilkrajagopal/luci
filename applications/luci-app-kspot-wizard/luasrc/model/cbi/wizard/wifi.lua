...
m:chain("network")
m:chain("firewall")
m.redirect = luci.dispatcher.build_url("admin/wizard/wireless")
...
if not wnet or not wdev then
	luci.http.redirect(luci.dispatcher.build_url("admin/wizard/wireless"))
	return
end
...
function m.parse(map)
	...
	if m:formvalue("cbid.wireless.%s.__toggle" % wdev:name()) then
		...
		luci.http.redirect(luci.dispatcher.build_url("admin/wizard/wireless", arg[1]))
		return
	end
